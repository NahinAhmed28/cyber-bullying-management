...
class CentralCommitteeController extends Controller
{
  ...
  const VIEW_PATH = 'admin.central_committee.';
  ...
  {
    //dd($request->all());
    $admins = CentralCommittee::with('role','userType','association','area','branch')->utwd()->orderBy('id','desc')->paginate(100);
    return view(self::VIEW_PATH . 'index',compact('admins'));
  }

  public function create()
  {
    //$this->authorize('create',CentralCommittee::class);
    
    $user_types = UserType::utwd()->where(['id'=>Admin::CenterCommittee])->get();
    $roles = Role::utwd()->where(['id'=>Admin::CenterCommittee])->get();
  ...
  public function edit(Request $request, $id)
  {
    ...
    //$this->authorize('update',CentralCommittee::class);
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      ...
      $admin = CentralCommittee::where(['id'=>$id])->first();
      ...
      CentralCommittee::find($id)->update($data);
      Cache::forget('locWiseAuthUserInfo');
    } catch (\Throwable $exception) {
      return back()->with([
        //'error' => __('admin.common.error'),
  ...
  {
    //$this->authorize('delete',CentralCommittee::class);

    $admin = CentralCommittee::find($id);
    if ( is_null($admin) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
        'alert-type' => 'error'
