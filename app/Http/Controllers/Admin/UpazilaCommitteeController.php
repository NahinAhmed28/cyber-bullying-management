...
use App\Models\UpazilaCommittee;
...
class UpazilaCommitteeController extends Controller
{
  
  const VIEW_PATH = 'admin.upazila_committee.';
  ...
  {
    //dd($request->all());
    $admins = UpazilaCommittee::with('role','userType','association','area','branch')->utwd()->orderBy('id','desc')->paginate(100);
    return view(self::VIEW_PATH . 'index',compact('admins'));
  }

  public function create()
  {
    //$this->authorize('create',UpazilaCommittee::class);
    
    $user_types = UserType::utwd()->where(['id'=>Admin::UpazilaCommittee])->get();
    $roles = Role::utwd()->where(['id'=>Admin::UpazilaCommittee])->get();
  ...
  public function edit(Request $request, $id)
  {
    ...
    //$this->authorize('update',UpazilaCommittee::class);
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      ...
      $admin = UpazilaCommittee::where(['id'=>$id])->first();
      ...
      UpazilaCommittee::find($id)->update($data);
      Cache::forget('locWiseAuthUserInfo');
    } catch (\Throwable $exception) {
      return back()->with([
        //'error' => __('admin.common.error'),
  ...
  {
    //$this->authorize('delete',UpazilaCommittee::class);

    $admin = UpazilaCommittee::find($id);
    if ( is_null($admin) == true) {
      return back()->with([
        'error' => __('admin.common.error'),
        'alert-type' => 'error'
