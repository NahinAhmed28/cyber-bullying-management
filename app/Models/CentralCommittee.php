...
class CentralCommittee extends Authenticatable
{
    ...
    public function scopeUtwd($query) {
        ...
        if($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[9]){
            return $query->where('user_type_id',Admin::CenterCommittee)->where(['created_by_id'=>$authUser->created_by_id]);
            ...
            return $query->where('user_type_id',Admin::CenterCommittee)->where(['upazila_id'=>$authUser->upazila_id]);
        } elseif($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[6]){
            return $query->where('user_type_id',Admin::CenterCommittee)->where(['district_id'=>$authUser->district_id]);
        } elseif($authUser->userType->default_role == Admin::DEFAULT_ROLE_LIST[5]){
            return $query->where('user_type_id',Admin::CenterCommittee)->where(['district_id'=>$authUser->district_id]);
        } else{
            return $query->where('user_type_id',Admin::CenterCommittee);
        }
