...
@section('breadcrumb')
  @include('../admin.layouts.partials.breadcrumb', ['path1' => __('admin.menu.case'), 'route1' => route('admin.case') ])
@endsection
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-10">
            <h1 class="text-light">
              <i class="fas fa-bookmark"></i> {{ __('admin.menu.case') }}
            </h1>
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="text-bold row text-center border py-2">
                      <div class="col-md-3">
                          <a href="{{route('admin.case')}}" style="color: {{$retVal = (Route::current()->getName() == 'admin.case') ? 'blue' : 'black' }} ">
                            {{ (app()->getLocale() == 'en') ? 'All Cases' : 'সব কেস' }} -
                            ( {{(app()->getLocale() == 'en') ? $totalApplication : dateFormatEnglishToBanglaHlp($totalApplication) }} )
                          </a>
                      </div>
                      @foreach($caseTypesWithCount as $caseType)
                          <div class="col-md-3">
                              <a href="{{ route('admin.case', ['cid' => $caseType->id]) }}"
                                 style="color: {{ (Route::currentRouteName() == 'admin.case' && Route::current()->parameter('cid') == $caseType->id) ? 'blue' : 'black' }};
                                    border-bottom: {{ (Route::currentRouteName() == 'admin.case' && Route::current()->parameter('cid') == $caseType->id) ? '2px solid #6E4798' : 'none' }}">
                                  {{ $caseType->{'title_' . app()->getLocale()} }} -
                                  ({{ (app()->getLocale() == 'en') ? $caseType->applications_count : dateFormatEnglishToBanglaHlp($caseType->applications_count) }})
                              </a>
                          </div>
                      @endforeach
                  </div>
                  ...
                  <div style="overflow-x:auto;" style="border-radius: 25px !important;">
                      <table id="example1" class="table table-hover">
                          ...
                          <tr style="background-color: #6E4798;" class="text-light">
                              ...
                              <th>{{ __('admin.case.case_status') }}</th>
                              <th>{{ __('admin.case.download_pdf') }}</th>
                              <th>{{ __('admin.case.action') }}</th>
                          </tr>
                          </thead>
                          <tbody>
                          @foreach ($applications as $key => $application)
                              <tr onclick="redirectToEdit(event, '{{ route('admin.case.show', ['id' => @$application->id]) }}')">
                                          ...
                                          <td>
                                              <div class="dropdown" id="dropdownContainer">
                                                  ...
                                                  <div class="dropdown-menu">
                                                      @can('update', app('App\Models\Applicationn'))
                                                          <a class="btn btn-xs btn-primary" href="{{ route('admin.case.edit', $application->id) }}">
                                                              <i class="fas fa-edit"></i> {{ __('admin.common.edit') }}
                                                          </a>
                                                      @endcan

                                                      @can('delete', app('App\Models\Applicationn'))
                                                          <a class="btn btn-xs btn-danger" href="{{ route('admin.case.delete', $application->id) }}" data-href="{{ route('admin.case.delete', [$application->id, 1]) }}" class="delete">
                                                              <i class="fas fa-trash-alt"></i> {{ __('admin.common.delete') }}
                                                          </a>
                                                      @endcan
                                                      @can('edit_step_info', app('App\Models\Applicationn'))
                                                          <a href="{{ route('admin.case.edit_step_info', $application->id) }}" class=" btn btn-xs btn-warning">
                                                              {{ __('admin.case.update_step_info_ans') }}
                                                          </a>
                                                      @endcan
                                                      @can('edit_addmember_info', app('App\Models\Applicationn'))
                              ...
                              <tr onclick="redirectToEdit(event, '{{ route('admin.case.show', ['id' => @$application->id]) }}')">
                                  ...
                                  <td colspan="2"> {{ __('admin.case.case_category') }} : <br>
                                  ...
                                  <td colspan="2"> <span class="font-weight-bolder">{{ __('admin.case.timer') }} : </span> <br>
                                      <span class="p-1 border border-secondary font-weight-bold" style="color: #05003A">
