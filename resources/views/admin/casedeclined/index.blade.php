...
@section('breadcrumb')
  @include('../admin.layouts.partials.breadcrumb', ['path1' => __('admin.menu.casedeclined'), 'route1' => route('admin.casedeclined') ])
@endsection
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-12">
            <h1 class="text-light">
              <i class="fas fa-bookmark"></i> {{ __('admin.menu.casedeclined') }}
            </h1>
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="text-bold row text-center border py-2">
                      <div class="col-md-3">
                          <a href="{{route('admin.casedeclined')}}" style="color: {{$retVal = (Route::current()->getName() == 'admin.casedeclined') ? 'blue' : 'black' }} ">
                            {{ __('admin.menu.cases') }} -
                      ...
                      @foreach($caseTypesWithCount as $caseType)
                          <div class="col-md-3">
                              <a href="{{ route('admin.casedeclined', ['cid' => $caseType->id]) }}"
                                 style="color: {{ (Route::currentRouteName() == 'admin.casedeclined' && Route::current()->parameter('cid') == $caseType->id) ? 'blue' : 'black' }};
                                    border-bottom: {{ (Route::currentRouteName() == 'admin.casedeclined' && Route::current()->parameter('cid') == $caseType->id) ? '2px solid #6E4798' : 'none' }}">
                                  {{ $caseType->{'title_' . app()->getLocale()} }} -
                                  ({{ (app()->getLocale() == 'en') ? $caseType->declinedapplications_count : dateFormatEnglishToBanglaHlp($caseType->declinedapplications_count) }})
                              </a>
                          </div>
                      @endforeach
                  </div>
                  ...
                  <div style="overflow-x:auto;" style="border-radius: 25px !important;">
                      <table id="example1" class="table table-hover">
                          ...
                          <tr style="background-color: #6E4798;" class="text-light">
                              ...
                              <th>{{ __('admin.casedeclined.case_status') }}</th>
                              <th>{{ __('admin.casedeclined.download_pdf') }}</th>
                              <th>{{ __('admin.casedeclined.action') }}</th>
                          </tr>
                          </thead>
                          <tbody>
                          @foreach ($applications as $key => $application)
                              <tr onclick="redirectToEdit(event, '{{ route('admin.casedeclined.show', ['id' => @$application->id]) }}')">
                                              ...
                                              <div class="dropdown" id="dropdownContainer">
                                                  ...
                                                  <div class="dropdown-menu">
                                                      @can('update', app('App\Models\Casedeclined'))
                                                          <a class="btn btn-xs btn-primary" href="{{ route('admin.casedeclined.edit', $application->id) }}">
                                                              <i class="fas fa-edit"></i> {{ __('admin.common.edit') }}
                                                          </a>
                                                      @endcan

                                                      @can('delete', app('App\Models\Casedeclined'))
                                                          <a class="btn btn-xs btn-danger" href="{{ route('admin.casedeclined.delete', $application->id) }}" data-href="{{ route('admin.casedeclined.delete', [$application->id, 1]) }}" class="delete">
                              ...
                              <tr onclick="redirectToEdit(event, '{{ route('admin.casedeclined.show', ['id' => @$application->id]) }}')">
                                  ...
                                  <td colspan="2"> {{ __('admin.casedeclined.case_category') }} : <br>
                                  ...
                                  <td colspan="2"> <span class="font-weight-bolder">{{ __('admin.casedeclined.timer') }} : </span> <br>
                                      <span class="p-1 border border-secondary font-weight-bold" style="color: #05003A">
