...
@section('breadcrumb')
  @include('../admin.layouts.partials.breadcrumb', ['path1' => __('admin.menu.caseincomplete'), 'route1' => route('admin.caseincomplete') ])
@endsection
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-12">
            <h1 class="text-light">
              <i class="fas fa-bookmark"></i> {{ __('admin.menu.caseincomplete') }}
            </h1>
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="text-bold row text-center border py-2">
                      <div class="col-md-3">
                          <a href="{{route('admin.caseincomplete')}}" style="color: {{$retVal = (Route::current()->getName() == 'admin.caseincomplete') ? 'blue' : 'black' }} ">
                            {{ __('admin.menu.cases') }} -
                      ...
                      @foreach($caseTypesWithCount as $caseType)
                          <div class="col-md-3">
                              <a href="{{ route('admin.caseincomplete', ['cid' => $caseType->id]) }}"
                                 style="color: {{ (Route::currentRouteName() == 'admin.caseincomplete' && Route::current()->parameter('cid') == $caseType->id) ? 'blue' : 'black' }};
                                    border-bottom: {{ (Route::currentRouteName() == 'admin.caseincomplete' && Route::current()->parameter('cid') == $caseType->id) ? '2px solid #6E4798' : 'none' }}">
                                  {{ $caseType->{'title_' . app()->getLocale()} }} -
                                  ({{ (app()->getLocale() == 'en') ? $caseType->incompleteapplications_count : dateFormatEnglishToBanglaHlp($caseType->incompleteapplications_count) }})
                              </a>
                          </div>
                      @endforeach
                  </div>
                  ...
                  <div style="overflow-x:auto;" style="border-radius: 25px !important;">
                      <table id="example1" class="table table-hover">
                          ...
                          <tr style="background-color: #6E4798;" class="text-light">
                              ...
                              <th>{{ __('admin.caseincomplete.case_status') }}</th>
                              <th>{{ __('admin.caseincomplete.download_pdf') }}</th>
                              <th>{{ __('admin.caseincomplete.action') }}</th>
                          </tr>
                          </thead>
                          <tbody>
                          @foreach ($applications as $key => $application)
                              <tr onclick="redirectToEdit(event, '{{ route('admin.caseincomplete.show', ['id' => @$application->id]) }}')">
                                              ...
                                              <div class="dropdown" id="dropdownContainer">
                                                  ...
                                                  <div class="dropdown-menu">
                                                      @can('update', app('App\Models\Caseincomplete'))
                                                          <a class="btn btn-xs btn-primary" href="{{ route('admin.caseincomplete.edit', $application->id) }}">
                                                              <i class="fas fa-edit"></i> {{ __('admin.common.edit') }}
                                                          </a>
                                                      @endcan

                                                      @can('delete', app('App\Models\Caseincomplete'))
                                                          <a class="btn btn-xs btn-danger" href="{{ route('admin.caseincomplete.delete', $application->id) }}" data-href="{{ route('admin.caseincomplete.delete', [$application->id, 1]) }}" class="delete">
                              ...
                              <tr onclick="redirectToEdit(event, '{{ route('admin.caseincomplete.show', ['id' => @$application->id]) }}')">
                                  ...
                                  <td colspan="2"> {{ __('admin.caseincomplete.case_category') }} : <br>
                                  ...
                                  <td colspan="2"> <span class="font-weight-bolder">{{ __('admin.caseincomplete.timer') }} : </span> <br>
                                      <span class="p-1 border border-secondary font-weight-bold" style="color: #05003A">
