...
@section('breadcrumb')
  @include('../admin.layouts.partials.breadcrumb', ['path1' => __('admin.menu.casepending'), 'route1' => route('admin.casepending') ])
@endsection
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-12">
            <h1 class="text-light">
              <i class="fas fa-bookmark"></i> {{ __('admin.menu.casepending') }}
            </h1>
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="text-bold row text-center border py-2">
                      <div class="col-md-3">
                          <a href="{{route('admin.casepending')}}" style="color: {{$retVal = (Route::current()->getName() == 'admin.casepending') ? 'blue' : 'black' }} ">
                            {{ __('admin.menu.cases') }} -
                      ...
                      @foreach($caseTypesWithCount as $caseType)
                          <div class="col-md-3">
                              <a href="{{ route('admin.casepending', ['cid' => $caseType->id]) }}"
                                 style="color: {{ (Route::currentRouteName() == 'admin.casepending' && Route::current()->parameter('cid') == $caseType->id) ? 'blue' : 'black' }};
                                    border-bottom: {{ (Route::currentRouteName() == 'admin.casepending' && Route::current()->parameter('cid') == $caseType->id) ? '2px solid #6E4798' : 'none' }}">
                                  {{ $caseType->{'title_' . app()->getLocale()} }} -
                                  ({{ (app()->getLocale() == 'en') ? $caseType->pendingapplications_count : dateFormatEnglishToBanglaHlp($caseType->pendingapplications_count) }})
                              </a>
                          </div>
                      @endforeach
                  </div>
                  ...
                  <div style="overflow-x:auto;" style="border-radius: 25px !important;">
                      <table id="example1" class="table table-hover">
                          ...
                          <tr style="background-color: #6E4798;" class="text-light">
                              ...
                              <th>{{ __('admin.casepending.case_status') }}</th>
                              <th>{{ __('admin.casepending.download_pdf') }}</th>
                              <th>{{ __('admin.casepending.action') }}</th>
                          </tr>
                          </thead>
                          <tbody>
                          @foreach ($applications as $key => $application)
                              <tr onclick="redirectToEdit(event, '{{ route('admin.casepending.show', ['id' => @$application->id]) }}')">
                                              ...
                                              <div class="dropdown" id="dropdownContainer">
                                                  ...
                                                  <div class="dropdown-menu">
                                                      @can('update', app('App\Models\Casepending'))
                                                          <a class="btn btn-xs btn-primary" href="{{ route('admin.casepending.edit', $application->id) }}">
                                                              <i class="fas fa-edit"></i> {{ __('admin.common.edit') }}
                                                          </a>
                                                      @endcan

                                                      @can('delete', app('App\Models\Casepending'))
                                                          <a class="btn btn-xs btn-danger" href="{{ route('admin.casepending.delete', $application->id) }}" data-href="{{ route('admin.casepending.delete', [$application->id, 1]) }}" class="delete">
                              ...
                              <tr onclick="redirectToEdit(event, '{{ route('admin.casepending.show', ['id' => @$application->id]) }}')">
                                  ...
                                  <td colspan="2"> {{ __('admin.casepending.case_category') }} : <br>
                                  ...
                                  <td colspan="2"> <span class="font-weight-bolder">{{ __('admin.casepending.timer') }} : </span> <br>
                                      <span class="p-1 border border-secondary font-weight-bold" style="color: #05003A">
